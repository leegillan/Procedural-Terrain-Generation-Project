...
{
	initShader(L"TerrainShader_vs.cso", L"TerrainShader_ps.cso");
}
...
void TerrainShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix, const XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* grassTexture, ID3D11ShaderResourceView* snowTexture, ID3D11ShaderResourceView* rockTexture, ID3D11ShaderResourceView* sandTexture, Light* light, Timer* timer, XMFLOAT4 multiPosition[], XMFLOAT4 multiDiffuse[], float height)
{
	HRESULT result;
	...
	deviceContext->PSSetShaderResources(3, 1, &sandTexture);
